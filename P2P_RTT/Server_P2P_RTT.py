...
def respond_to_client(conn_socket, client_address):
    ...
    while True:
        ...
        if not head:
            """servers_wth_port_socket.pop(conn_socket.getpeername()[1])
            servers_wth_port_ip.pop(conn_socket.getpeername()[1])"""
            conn_socket.close()
            break
        ...
        if type == 0:
            ...
            if client_address[1] > 5559 or client_address[1] < 5555:
                servers_wth_port_socket.pop(conn_socket.getpeername()[1])
                servers_wth_port_ip.pop(conn_socket.getpeername()[1])
        elif type == 1:
            ...
            if len(receiver.split('\0')[0]) == len(receiver):
                if receiver.strip() in users.keys():
                    ...
                    try:
                        users[receiver.strip()].send(
                            struct.pack(">bbhh", type, 0, len(data), len(data) - len(message)) + data.encode())
                        print('Message received from {0}\n\t Transfered to {1}'.format(client_address, users[
                            receiver.strip()].getpeername()))
            ...
            conn_socket.send(struct.pack(">bbhh", 4, 0, 0, 0))
...
if degel == 0:
    threading.Thread(target=respond_to_client,
                     args=(servers_wth_port_socket[goodport], ('127.0.0.1', goodport))).start()
